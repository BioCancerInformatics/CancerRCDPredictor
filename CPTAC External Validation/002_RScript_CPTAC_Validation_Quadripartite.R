...
# Panel labels and log-rank p-values (collected from above)
p_values <- list(
  "MVL SuperLearner" = if(exists("lr_test")) 1 - pchisq(lr_test$chisq, df=2) else NA,
  "XGBoost" = NA, "RSF" = NA, "Boruta" = NA, "MTLR" = NA
)

# Individual log-rank from per-learner loop (collect from log)
p_vals_from_log <- sapply(c("XGBoost","RSF","MTLR","Boruta"), function(l) {
  pat <- sprintf("%s.*KM log-rank p", l)
  line <- grep(pat, readLines(file.path(OUT_DIR, "cptac_validation_quadripartite.log")), value=TRUE)
  if(length(line)) as.numeric(sub(".*=\\s*", "", line)) else NA
})
names(p_vals_from_log) <- c("XGBoost","RSF","MTLR","Boruta")

learners_mp <- list(
  list(name="MVL SuperLearner", col="SuperLearner_Risk", panel="A", p=p_values[["MVL SuperLearner"]]),
  list(name="RSF", col="RSF_Risk", panel="B", p=p_vals_from_log["RSF"]),
  list(name="XGBoost", col="XGBoost_Risk", panel="C", p=p_vals_from_log["XGBoost"]),
  list(name="Boruta-RSF", col="Boruta_Risk", panel="D", p=p_vals_from_log["Boruta"]),
  list(name="MTLR", col="MTLR_Risk", panel="E", p=p_vals_from_log["MTLR"])
)

# Colors for publication
pub_cols <- c("#2166AC", "#F4A582", "#B2182B")

# PDF
pdf(file.path(OUT_DIR, "cptac_km_multipanel.pdf"), width=14, height=16)
par(mfrow=c(3,2), mar=c(4.5,4.5,3,2), oma=c(0,0,2,0))
for (li in learners_mp) {
  risk <- as.numeric(as.character(all_cptac[[li$col]]))
  tert <- cut(risk, breaks=quantile(risk, probs=c(0,1/3,2/3,1), na.rm=TRUE),
              labels=c("Low","Medium","High"), include.lowest=TRUE)
  km <- survfit(Surv(OS.time, OS) ~ tert, data=all_cptac)
  plot(km, col=pub_cols, lwd=2.5, xlab="Days", ylab="OS Probability",
       main=sprintf("%s. %s", li$panel, li$name), cex.main=1.1, mark.time=FALSE, conf.int=FALSE)
  p_txt <- if(!is.na(li$p)) sprintf("p = %.6f", li$p) else "p = NA"
  if(!is.na(li$p) && li$p < 0.0001) p_txt <- "p < 0.0001"
  text(x=max(all_cptac$OS.time,na.rm=TRUE)*0.02, y=0.12, p_txt, pos=4, cex=0.9, font=2)
  legend("topright", c("Low Risk","Medium Risk","High Risk"), col=pub_cols, lwd=2.5, cex=0.7, bty="n")
}
# Panel F — Delta bar chart
par(mar=c(6,4,3,1))
delta_mat <- as.matrix(delta_table[,2:6])
rownames(delta_mat) <- delta_table$Cancer
delta_mat[delta_mat == "NA"] <- NA
delta_mat <- apply(delta_mat, 2, as.numeric)
bp <- barplot(t(delta_mat), beside=TRUE, col=c("#B2182B","#2166AC","#F4A582","#4DAF4A","#984EA3"),
              main="F. Internal-External Delta by Learner", ylab="Delta (TCGA - CPTAC)",
              las=2, cex.names=0.7, border=NA,
              legend.text=c("XGBoost","RSF","MTLR","Boruta","MVL"),
              args.legend=list(x="topleft", cex=0.65, bty="n", ncol=2))
abline(h=0, lty=2, col="gray50")
dev.off()

# TIFF 600 dpi
tiff(file.path(OUT_DIR, "cptac_km_multipanel.tiff"), width=14, height=16, units="in", res=600, compression="lzw")
par(mfrow=c(3,2), mar=c(4.5,4.5,3,2), oma=c(0,0,2,0))
for (li in learners_mp) {
  risk <- as.numeric(as.character(all_cptac[[li$col]]))
  tert <- cut(risk, breaks=quantile(risk, probs=c(0,1/3,2/3,1), na.rm=TRUE),
...
cat("  Multi-panel saved: cptac_km_multipanel.pdf + .tiff\n")
