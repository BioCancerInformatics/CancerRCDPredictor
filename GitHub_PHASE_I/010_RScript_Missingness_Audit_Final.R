...
#   (here: df377), applying demographic/clinical and omic-layer constraints before
...
#
# OUTPUTS:
#   - df377_missingness_by_type_STRICT.tsv
...
#   - df377_missingness_boundary_EQ_threshold.tsv
...
# ---- Commentary on eligibility results based on df377 ----
...
file_path <- "D:/Pré-artigo 5-optosis model/Machine learning CancerRCDShiny_prediction/Enrique version/PHASE_II/dfXXX_series/df377.rds"

# Import the RDS object
df377 <- readRDS(file_path)

# Inspect object class
class(df377)

# Check dimensions (if data.frame/tibble/matrix)
dim(df377)
...
empty_string_counts <- sapply(df377, function(col) sum(col == "", na.rm = TRUE))
...
if (length(changed_idx)) {
  df377$DSS[changed_idx] <- 0L
  # (optional) provenance flag
  df377$DSS_harmonized_from_OS0 <- FALSE
  df377$DSS_harmonized_from_OS0[changed_idx] <- TRUE
}

# replace the offending line with a local, read-only cast
dss_int <- suppressWarnings(as.integer(df377$DSS))
# …use dss_int in whatever summary/check follows…


## Reorder specific variables in df377
## Objective:
...
saveRDS(df377, file = "df377.rds")

# 🔄 Reload dataset (restores the exact same object)
df377 <- readRDS("df377.rds")
...
# ------------------ Input checks -------------------
stopifnot(exists("df377"))
stopifnot(ncol(df377) >= 63)
stopifnot("type" %in% names(df377))

# ------------------ Prep ---------------------------
DT <- as.data.table(df377)
...
if (exists("safe_export_tsv", mode = "function")) {
  safe_export_tsv(missingness_long_strict, "df377_missingness_by_type_STRICT.tsv")
} else if (requireNamespace("data.table", quietly = TRUE)) {
  data.table::fwrite(missingness_long_strict, "df377_missingness_by_type_STRICT.tsv",
                     sep = "\t", na = "NA", quote = FALSE)
} else {
  write.table(missingness_long_strict, "df377_missingness_by_type_STRICT.tsv",
...
safe_export_tsv(as.data.frame(eligibility_counts), "df377_eligibility_counts.tsv")  # corrected spelling
...
if (nrow(boundary_hits) > 0L) {
  ...
  fwrite(boundary_hits, "df377_missingness_boundary_EQ_threshold.tsv",
...
#
# Outputs:
#   - df377_survival_patch_gate_typepair.tsv        (per-type gating table)
#   - df377_survival_patch_<PAIR>_rowwise_gate.tsv  (row-wise flags per pair)
...
run_survival_pair_patch <- function(df, elig_tbl,
                                    ...
                                    max_age_yrs = MAX_AGE_YRS,
                                    days_per_year = DAYS_PER_YR,
                                    export_prefix = "df377_survival_patch") {
  req <- c("type", year_col)
...
#
# df377_survival_patch_gate_typepair.tsv
...
## ===============================================================================================================
#' Exclude df377 columns flagged as ineligible (omic group) in STRICT table
## Exclude df377 columns using strict table where: group == "omic" & eligible_for_imputation == FALSE
## Outputs:
##   - df377_filtered_out_OMIC_FALSE_INELIGIBLE.rds
##   - df377_excluded_variables.tsv
##   - df377_exclusion_summary.tsv
##   - (optional) df377_exclusion_flagged_not_in_df377.tsv
## =============================================================================
#' Exclude df377 columns flagged as ineligible (omic group) in STRICT table
#'
#' @description
#' Loads `df377.rds`, reads the strict eligibility table, selects rows where
#' `group == "omic"` **and** `eligible_for_imputation == FALSE`, and drops the
#' corresponding `variable`-named columns from `df377`. Emits reproducible audit
#' artifacts and saves the filtered object.
#'
#' @param strict_path Character. Path to the strict eligibility TSV.
#'   Defaults to `"df377_missingness_by_type_STRICT.tsv"`. If not found, a
#'   fallback to `..._STRIC.tsv` is attempted.
#' @param df_rds Character. Path to the source object `"df377.rds"`.
#' @param prefix Character. File prefix for outputs (e.g., `"df377"`).
#'
#' @return (Invisibly) the filtered `data.frame` (`df377_filtered`).
#'
#' @details
...
#' column must match column names in `df377` exactly; variables flagged in the
#' strict table but absent from `df377` are reported separately and ignored.
#'
...
#' - `<prefix>_exclusion_flagged_not_in_df377.tsv` — variables flagged in STRICT
#'   but not present in `df377` (emitted only if non-empty).
#'
#' **Robustness:**
...
#' @examples
#' \dontrun{
#' run_df377_omic_ineligible_exclusion()
#' run_df377_omic_ineligible_exclusion(
#'   strict_path = "df377_missingness_by_type_STRICT.tsv",
#'   df_rds      = "df377.rds",
#'   prefix      = "df377"
#' )
#' }
run_df377_omic_ineligible_exclusion <- function(
    strict_path = "df377_missingness_by_type_STRICT.tsv",
    df_rds      = "df377.rds",
    prefix      = "df377"
) {
  ...
  # ========================== Load source object ===============================
  # Single source of truth: df377 from RDS. Keeps this function idempotent and
  # independent of the caller's workspace state.
  df377 <- safe_readRDS(df_rds)
  original_ncol <- ncol(df377)
  ...
  # ================== Build exclusion list present in df377 ===================
  # Prevents accidental errors when STRICT contains variables not in df377.
  vars_flagged      <- unique(as.character(strict_excl$variable))
  vars_to_remove    <- intersect(vars_flagged, names(df377))
  vars_not_in_df377 <- setdiff(vars_flagged, names(df377))
  
  # ======================== Apply exclusion and audit =========================
  df377_filtered <- df377[, setdiff(names(df377), vars_to_remove), drop = FALSE]
  remaining_ncol <- ncol(df377_filtered)
  ...
  if (length(vars_not_in_df377)) {
    safe_export_tsv(
      data.frame(variable = sort(vars_not_in_df377),
                 note = "flagged_in_STRICT_but_not_in_df377",
                 stringsAsFactors = FALSE),
      paste0(prefix, "_exclusion_flagged_not_in_df377.tsv")
    )
  }
  saveRDS(df377_filtered, paste0(prefix, "_filtered_out_OMIC_FALSE_INELIGIBLE.rds"))
  
  # ============================== Log =========================================
  log_msg(sprintf("Excluded %d 'omic' variables. %s: %d -> %d columns.",
                  length(vars_to_remove), prefix, original_ncol, remaining_ncol))
  
  invisible(df377_filtered)
}
...
## ---------------------------------------------------------------------------
## Import df377_filtered_out_OMIC_FALSE_INELIGIBLE.rds (robust)
...
# ---- Load into "new" fresh df377 and report fro downstream analysis
df377 <- safe_readRDS("df377_filtered_out_OMIC_FALSE_INELIGIBLE.rds")
log_msg("Loaded df377_filtered_out_OMIC_FALSE_INELIGIBLE.rds -> 'df377' with ",
        nrow(df377), " rows and ", ncol(df377), " columns.")
