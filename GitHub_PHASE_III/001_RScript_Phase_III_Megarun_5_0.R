# PHASE III — SUPERVISED MACHINE LEARNING FOR OUTCOME PREDICTION (NON-PH)
# Quadripartite Meta-Learner: RSF, XGBoost, Boruta, and MTLR
###############################################################################

# -------------------------------------------------------------------------
# PREAMBLE — Phase III Constitution & Logic
# -------------------------------------------------------------------------
# This script represents the final predictive modeling stage (Phase III).
# As determined by the Phase II CANARY evaluation (CoxNet), the vast majority of 
# predictive structures strongly violate the proportional hazards geometry. 
# Thus, Phase III is strictly restricted to handling non-proportional, nonlinear 
# survival topologies via ensemble Machine Learning methods. 
#
# CONSTRAINTS:
...
results <- future_lapply(1:nrow(dfinput), function(i) {
  ...
  tryCatch({
    ...
    if(!is.null(shap_df)) {
       ...
       suppressWarnings({
         ...
         if(!is.null(shp)) {
            ...
            # 2. Dependence Plots (All contributing signatures)
            top_signatures <- shap_df$Feature[shap_df$Mean_Abs_SHAP > 0]
            ...
            # True Precision Oncology: All cohort patients
            for(i in 1:nrow(X_matrix)) {
               p_id <- rownames(X_matrix)[i]
               if(is.null(p_id) || p_id == "") p_id <- as.character(i)
               target_patients[[paste0("Patient_Trajectory_", p_id)]] <- i
            }
            ...
            for(pt_label in names(target_patients)) {
               ...
               # [CLINICAL AUDIT SUTURE]: Natively export raw mathematical trajectory arrays alongside the Waterfall graphics
               tryCatch({
                  raw_sv <- shap_vals[pid, ]
                  raw_sv <- raw_sv[names(raw_sv) != "BIAS"]
                  sv_ordered <- raw_sv[order(abs(raw_sv), decreasing = TRUE)]
                  df_traj <- data.frame(Feature = names(sv_ordered), SHAP_Value = as.numeric(sv_ordered), stringsAsFactors = FALSE)
                  write.csv(df_traj, file.path(shap_vis_dir, paste0(unit_id, "_SHAP_Decision_", pt_label, "_Trajectory_Data.csv")), row.names=FALSE)
               }, error = function(e) NULL)
            }
         }
       })
    ...
    log_file_path_atomic <- file.path(WORKING_DIR, "PHASE_III_Execution_Audit.log")
    cat(paste0(Sys.time(), " - ", success_msg, "\n"), file = log_file_path_atomic, append = TRUE)
    
    # 2. Append tabular Performance CSV dynamically
    csv_file_path_atomic <- file.path(WORKING_DIR, "MASTER_Phase_III_Performance.csv")
...
log_file_path <- file.path(WORKING_DIR, "PHASE_III_Execution_Audit.log")
...
if (length(perf_files) > 0) {
  ...
  master_save_path <- file.path(WORKING_DIR, "MASTER_Phase_III_Performance.csv")
...
# =========================================================================
# IX. AUTONOMOUS ZIMA C-SUITE TRIGGER
# =========================================================================
message("🚀 Launching Phase III ZIMA C-Suite Generator autonomously...")
source("RScript_PHASE_III_ZIMA_C_Suite_Generator.R")
