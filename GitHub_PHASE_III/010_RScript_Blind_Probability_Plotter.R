# =========================================================================
# ZIMA BLIND CLINICAL PROBABILITY PLOTTER
...
PROB_DIR <- file.path(ZIMA_ROOT, "CancerRCDShiny_Blind_Clinical_Probabilities")
PLOT_DIR <- file.path(ZIMA_ROOT, "Clinical_Probability_Plots")

if(!dir.exists(PLOT_DIR)) dir.create(PLOT_DIR, recursive=TRUE)

cat("\n[ZIMA Plotter] Loading 87 Clinical Probability Matrices...\n")

prob_files <- list.files(PROB_DIR, pattern = "_Clinical_Probabilities\\.tsv$", full.names = TRUE)
...
df_list <- lapply(prob_files, function(f) {
  d <- fread(f)
  unit_id <- gsub("_Clinical_Probabilities\\.tsv$", "", basename(f))
...
# Color path A Dark Grey, path B red
...
# 6. Export
out_tiff <- file.path(PLOT_DIR, "Clinical_Probability_Bifurcated_Composite.tiff")
out_pdf <- file.path(PLOT_DIR, "Clinical_Probability_Bifurcated_Composite.pdf")
...
cat(sprintf("✅ SUCCESS! Composite figure saved to: %s\n", PLOT_DIR))
